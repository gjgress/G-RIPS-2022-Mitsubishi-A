...
def FIS1(data_temp, method, wt_matrix = np.identity(6),plot = False, output_array_result = False):
    ...
    if (method ==1 and len(data_temp) != 4):
        ...
        return 0
    elif (method == 2 and len(data_temp) != 3):
        print('incorrect dimension of input')
        print(['dimension of data is ', len(data_temp), ' while using method 2']) 
        return 0  
    elif (method == 3 and len(data_temp) != 1):
        print('incorrect dimension of input')
        print(['dimension of data is ', len(data_temp), ' while using method 2'])
        return 0   
...
# +
        ...
        PD_fit_short = fuzz.interp_membership(x_PD, PD_short, data_temp[0]) 
        PD_fit_long = fuzz.interp_membership(x_PD, PD_long, data_temp[0])
        ...
        # Create weight rules : 
        weight[0,0]= PD_fit_short
        weight[0,1] = PD_fit_long
        ...
        output = output_array.sum()
    if output_array_result == False :
        return output
    elif output_array_result == True:
