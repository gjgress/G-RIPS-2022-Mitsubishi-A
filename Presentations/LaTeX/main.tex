...
%The benefit of these proposed mathematical formulations is their ability to be extended to include  to higher dimensional data 
...
%We also our method is valid in three-dimensional space as well because the vector connecting the trajectory points, the vector connecting the vertices of the road network, and the inner product of the two are all defined similarly in three-dimensional space. 
...
Then we define $P(v_{i}, v_{\alpha\beta}) := \left\langle\frac{v_{i}}{|v_{i}|}, \frac{v_{\alpha\beta}}{|v_{\alpha\beta}|}\right\rangle$ where $\langle-,-\rangle$ denotes the inner product on the Euclidean space $\mathbb{R}^{d}$.
Let $L_{i,e}$ be the distance between the trajectory point $p_{i}$ and the point on the edge $e$ which is closest to $p_{i}$.
We define the score
\begin{align*}
    S(e) := \sum_{i}\frac{P(v_{i}, v_{\alpha\beta})}{1+L_{i,e}}  
\end{align*}
for each edge.

We are also considering a methods that we use vectors representing the speed $\text{spd}_{i}$ and direction $u_{i}$ at the trajectory point $p_{i}$ instead of $v$. In this case, we can calculate the score of edge $e_{\alpha\beta}$ as follows:
\begin{align*}
    S(e) := \sum_{i}\frac{\text{spd}_{i}\langle u_{i}, v_{\alpha\beta}\rangle}{1+L_{i,e}}  
\end{align*}
